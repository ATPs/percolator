...
Caller::Caller()
    : pNorm_(NULL),
      ...
      outputEachFolder_(false),
      trainEach_(false),
      outputEachFolderPath_(""),
      outputEachTargetPsms_(false),
      outputEachDecoyPsms_(false),
      outputEachTargetPeptides_(false),
      outputEachDecoyPeptides_(false),
      outputEachExtraTab_(false),
      outputEachExtraXml_(false),
      outputEachExtraPepXml_(false),
      outputEachExtraWeight_(false),
      outputEachExtraProtein_(false),
      outputEachExtraDecoyProtein_(false),
      tabOutputTemplateFN_(""),
      xmlOutputTemplateFN_(""),
      pepXMLOutputTemplateFN_(""),
      weightOutputTemplateFN_(""),
      proteinResultTemplateFN_(""),
      decoyProteinResultTemplateFN_(""),
      resolvedInputOriginalFNs_(),
      perInputOutputSpecs_(),
      sourceTagDelimiter_(""),
      useMixMaxBase_(false),
      analytics_(true),
...
bool Caller::parseOptions(int argc, char** argv) {
  ...
  intro << greeter() << "\nUsage:\n";
  intro << "   percolator [-X pout.xml] [other options] pin.tsv [pin2.tsv ...]\n";
  intro << "   percolator [-X pout.xml] [other options] --tab-in pin.tsv\n";
  intro << "   percolator [-X pout.xml] [other options] --input-file-list "
           "inputs.txt\n";
  intro << "pin.tsv is the tab delimited output file generated by e.g. "
           "sqt2pin.\n";
  intro << "Tab input sources can be combined: positional files, --tab-in, "
           "and --input-file-list.\n";
  intro << "Patterns containing wildcard characters ('*', '?', '[]') are "
           "expanded internally.\n";
  intro << "Quote wildcard patterns to pass them literally to Percolator "
           "(example: \"*/*.pin.gz\").\n";
  ...
  intro << "Per-input output mode: use --output-each-folder for per-file "
           "result files, and --train-each to train each input independently.\n";
  intro << "Compatibility: existing single-file tab input commands remain "
           "unchanged.\n";
  ...
  endnote << "Input examples:\n";
  endnote << "  percolator file.pin\n";
  endnote << "  percolator file1.pin file2.pin\n";
  endnote << "  percolator --tab-in file.pin extra1.pin extra2.pin\n";
  endnote << "  percolator \"*/*.pin.gz\"\n";
  endnote << "  percolator --input-file-list inputs.txt\n";
  ...
  endnote << "  Leading/trailing whitespace is trimmed.\n";
  endnote << "  Blank lines are ignored.\n";
  endnote << "  Lines whose first non-space character is '#' are ignored.\n";
  endnote << "\n";
  endnote << "Mutual exclusivity:\n";
  endnote << "  --xml-in/--stdinput-xml cannot be combined with tab file "
             "inputs.\n";
  endnote << "  --stdinput-tab cannot be combined with tab file inputs.\n";
  endnote << "\n";
  endnote << "Per-input output mode:\n";
  endnote << "  --output-each-folder writes one output file per input.\n";
  endnote << "  Without --train-each: train once on combined input and split "
             "final outputs.\n";
  endnote << "  With --train-each: run full training/scoring per input.\n";
  endnote << "  File naming: <prefix>.target.psms.tsv, "
  ...
  // init
  CommandLineParser cmd(intro.str(), endnote.str());
  ...
  cmd.defineOption(
      ...
      "Cannot be combined with positional pin files, --tab-in, or "
      "--input-file-list.",
      "",
      TRUE_IF_SET);
  cmd.defineOption(
      ...
      "Cannot be combined with --xml-in or any tab input source.",
      "",
      TRUE_IF_SET);
  ...
  cmd.defineOption(
      "j", "tab-in",
      "[set by default] Add one pin-tab input path. Accepts regular file paths "
      "and literal wildcard patterns (when quoted). Can be combined with "
      "positional input files and --input-file-list.",
      "filename");
  cmd.defineOption("", "input-file-list",
                   "Read tab-input paths/patterns from a file. One entry per "
  ...
  cmd.defineOption("", "output-each-folder",
                   ...
                   "folder");
  cmd.defineOption("", "train-each",
                   "Only valid with --output-each-folder. Train and score "
                   "each input independently.",
                   "", TRUE_IF_SET);
  cmd.defineOption("k", "xml-in",
                   "Input file given in deprecated pin-xml format generated by "
                   "e.g. sqt2pin with the -k option. Cannot be combined with "
                   "--stdinput-xml or any tab input source.",
                   "filename");
  ...
  cmd.defineOption("r", "results-peptides",
                   ...
                   "of stdout (will be ignored if used with -U option). In "
                   "--output-each-folder mode the value is ignored and only "
                   "the option presence is used.",
                   "filename");
  cmd.defineOption("B", "decoy-results-peptides",
                   "Output tab delimited results for decoy peptides into a "
                   "file (will be ignored if used with -U option). In "
                   "--output-each-folder mode the value is ignored and only "
                   "the option presence is used.",
                   "filename");
  cmd.defineOption(
      "m", "results-psms",
      "Output tab delimited results of PSMs to a file instead of stdout. In "
      "--output-each-folder mode the value is ignored and only the option "
      "presence is used.",
      "filename");
  cmd.defineOption("M", "decoy-results-psms",
                   "Output tab delimited results for decoy PSMs into a file. "
                   "In --output-each-folder mode the value is ignored and only "
                   "the option presence is used.",
                   "filename");
  ...
  if (cmd.isOptionSet("output-each-folder")) {
    outputEachFolder_ = true;
    outputEachFolderPath_ = cmd.options["output-each-folder"];
  }
  if (cmd.isOptionSet("train-each")) {
    trainEach_ = true;
  }
  if (trainEach_ && !outputEachFolder_) {
    std::cerr << "Error: --train-each requires --output-each-folder."
              << std::endl;
    return 0;
  }
  ...
  if (cmd.isOptionSet("xmloutput")) {
    xmlOutputTemplateFN_ = cmd.options["xmloutput"];
    if (outputEachFolder_ && trainEach_) {
      outputEachExtraXml_ = true;
      xmlOutputFN_.clear();
    } else if (outputEachFolder_) {
      std::cerr << "Error: --xmloutput with --output-each-folder requires "
                << "--train-each." << std::endl;
      return 0;
    } else {
      xmlOutputFN_ = xmlOutputTemplateFN_;
      checkIsWritable(xmlOutputFN_);
    }
  }
  // pepXML
  if (cmd.isOptionSet("pepxml-output")) {
    pepXMLOutputTemplateFN_ = cmd.options["pepxml-output"];
    reportPepXML_ = true;
    if (outputEachFolder_ && trainEach_) {
      outputEachExtraPepXml_ = true;
      pepXMLOutputFN_.clear();
    } else if (outputEachFolder_) {
      std::cerr << "Error: --pepxml-output with --output-each-folder requires "
                << "--train-each." << std::endl;
      return 0;
    } else {
      pepXMLOutputFN_ = pepXMLOutputTemplateFN_;
  ...
  if (outputEachFolder_) {
    outputEachTargetPsms_ = cmd.isOptionSet("results-psms");
    outputEachDecoyPsms_ = cmd.isOptionSet("decoy-results-psms");
    outputEachTargetPeptides_ = cmd.isOptionSet("results-peptides");
    outputEachDecoyPeptides_ = cmd.isOptionSet("decoy-results-peptides");
  } else {
  ...
  if (cmd.isOptionSet("only-psms")) {
    ...
    if (outputEachFolder_) {
      if (outputEachTargetPeptides_ && VERB > 0) {
        cerr << "WARNING: --results-peptides is ignored with --only-psms in "
                "--output-each-folder mode."
             << endl;
      }
      if (outputEachDecoyPeptides_ && VERB > 0) {
        cerr << "WARNING: --decoy-results-peptides is ignored with --only-psms "
                "in --output-each-folder mode."
             << endl;
      }
      outputEachTargetPeptides_ = false;
      outputEachDecoyPeptides_ = false;
    ...
    if (outputEachFolder_) {
      // In this mode these options are enable-switches only; values are ignored.
      (void)cmd;
    } else {
      if (cmd.isOptionSet("results-peptides")) {
  ...
  const bool hasXmlInputFile = cmd.isOptionSet("xml-in");
  const bool hasXmlInputStdin = cmd.isOptionSet("stdinput-xml");
  const bool hasTabInputStdin = cmd.isOptionSet("stdinput-tab");

  if (hasXmlInputStdin && hasTabInputStdin) {
    std::cerr << "Error: use one of either --stdinput-xml or --stdinput-tab."
              << std::endl;
    return 0;
  }
  if (hasXmlInputFile && hasXmlInputStdin) {
    std::cerr << "Error: use one of either --xml-in or --stdinput-xml."
              << std::endl;
    return 0;
  }
  if ((hasXmlInputFile || hasXmlInputStdin) && hasTabInputStdin) {
    std::cerr << "Error: use one of either pin-xml or tab-delimited input "
              << "format." << std::endl;
    return 0;
  }
  ...
  std::vector<std::string> tabInputCandidates;
  if (cmd.isOptionSet("tab-in")) {
    tabInputCandidates.push_back(cmd.options["tab-in"]);
  }

  tabInputCandidates.insert(tabInputCandidates.end(), cmd.arguments.begin(),
                            cmd.arguments.end());

  if (cmd.isOptionSet("input-file-list")) {
    std::vector<std::string> listEntries;
    try {
      listEntries = InputFileResolver::readInputFileList(
          cmd.options["input-file-list"]);
    } catch (const std::exception& e) {
      std::cerr << e.what() << std::endl;
      return 0;
    }
    tabInputCandidates.insert(tabInputCandidates.end(), listEntries.begin(),
                              listEntries.end());
  }
  ...
  if (cmd.isOptionSet("tab-out")) {
    tabOutputTemplateFN_ = cmd.options["tab-out"];
    if (outputEachFolder_ && trainEach_) {
      outputEachExtraTab_ = true;
      tabOutputFN_.clear();
    } else if (outputEachFolder_) {
      std::cerr << "Error: --tab-out with --output-each-folder requires "
                << "--train-each." << std::endl;
      return 0;
    } else {
      tabOutputFN_ = tabOutputTemplateFN_;
      checkIsWritable(tabOutputFN_);
    }
  }

  if (cmd.isOptionSet("weights")) {
    weightOutputTemplateFN_ = cmd.options["weights"];
    if (outputEachFolder_ && trainEach_) {
  ...
  const bool hasTabInputSources = !tabInputCandidates.empty();
  const bool hasAnyInputSource =
      hasTabInputSources || hasXmlInputFile || hasXmlInputStdin ||
      hasTabInputStdin;
  if (!hasAnyInputSource) {
    cerr << "Error: too few arguments.";
    cerr << "\nInvoke with -h option for help\n";
    return 0;
  }

  if (hasTabInputSources) {
    if (hasXmlInputFile || hasXmlInputStdin) {
  ...
  if (cmd.isOptionSet("picked-protein")) {
    ...
    if (cmd.isOptionSet("results-proteins")) {
      proteinResultTemplateFN_ = cmd.options["results-proteins"];
      if (outputEachFolder_ && trainEach_) {
        outputEachExtraProtein_ = true;
        proteinResultFN_.clear();
      } else if (outputEachFolder_) {
        std::cerr << "Error: --results-proteins with --output-each-folder "
                  << "requires --train-each." << std::endl;
        return 0;
      } else {
        proteinResultFN_ = proteinResultTemplateFN_;
        checkIsWritable(proteinResultFN_);
      }
    }
    if (cmd.isOptionSet("decoy-results-proteins")) {
      decoyProteinResultTemplateFN_ = cmd.options["decoy-results-proteins"];
      if (outputEachFolder_ && trainEach_) {
        outputEachExtraDecoyProtein_ = true;
        decoyProteinResultFN_.clear();
      } else if (outputEachFolder_) {
        std::cerr << "Error: --decoy-results-proteins with --output-each-folder "
                  << "requires --train-each." << std::endl;
        return 0;
      } else {
        decoyProteinResultFN_ = decoyProteinResultTemplateFN_;
  ...
  const bool hasPerInputTsvOutputs =
      outputEachTargetPsms_ || outputEachDecoyPsms_ || outputEachTargetPeptides_ ||
      outputEachDecoyPeptides_;
  const bool hasPerInputExtraOutputs =
  ...
  if (outputEachFolder_) {
    if (!hasTabInputSources || hasXmlInputFile || hasXmlInputStdin ||
        hasTabInputStdin) {
      std::cerr << "Error: --output-each-folder requires tab file inputs "
                << "(positional, --tab-in, or --input-file-list)." << std::endl;
      return 0;
    }
...
void Caller::writePerInputResults(Scores& allScores, bool isUniquePeptideRun) {
  for (std::vector<PerInputOutputSpec>::const_iterator it =
           perInputOutputSpecs_.begin();
       it != perInputOutputSpecs_.end(); ++it) {
    std::string targetFN =
        isUniquePeptideRun ? it->targetPeptides : it->targetPsms;
    std::string decoyFN = isUniquePeptideRun ? it->decoyPeptides : it->decoyPsms;

    if (!targetFN.empty()) {
      ofstream targetStream(targetFN.c_str(), ios::out);
      allScores.printFiltered(LabelType::TARGET, targetStream, it->sourceKey,
                              sourceTagDelimiter_);
    }
    if (!decoyFN.empty()) {
      ofstream decoyStream(decoyFN.c_str(), ios::out);
      allScores.printFiltered(LabelType::DECOY, decoyStream, it->sourceKey,
                              sourceTagDelimiter_);
    }
  }
}
...
void Caller::writeResults(Scores& allScores,
  ...
  if (outputEachFolder_ && !trainEach_) {
    writePerInputResults(allScores, isUniquePeptideRun);
    return;
  }
...
//    postToPostHog("percolator_startup");
  ...
  perInputOutputSpecs_.clear();
  sourceTagDelimiter_.clear();
  resolvedInputOriginalFNs_.clear();

  if (!(tabInput_ && !readStdIn_)) {
    useMixMax_ = useMixMaxBase_;
    return runCore();
  }

  std::vector<std::string> expandedInputPaths;
  std::vector<std::string> materializedInputPaths;
  try {
    expandedInputPaths = InputFileResolver::expandPatterns(inputFNs_);
    expandedInputPaths = InputFileResolver::deduplicateKeepFirst(expandedInputPaths);
    materializedInputPaths = InputFileResolver::materializeGzipInputs(expandedInputPaths);
  } catch (const std::exception& e) {
    std::cerr << e.what() << std::endl;
    return 0;
  }

  resolvedInputOriginalFNs_ = expandedInputPaths;
  inputFNs_ = materializedInputPaths;
  ...
  std::string outputFolderError;
  if (!PerInputOutputPlanner::ensureOutputFolder(outputEachFolderPath_,
                                                 createdOutputFolder,
                                                 outputFolderError)) {
    std::cerr << outputFolderError << std::endl;
    return 0;
  }
  ...
  try {
    entries = PerInputOutputPlanner::buildEntries(expandedInputPaths,
                                                  materializedInputPaths);
  } catch (const std::exception& e) {
    std::cerr << e.what() << std::endl;
    return 0;
  }
  ...
  if (VERB > 0) {
    if (trainEach_) {
      std::cerr << "Per-input output mode enabled: each input will be trained "
                   "and scored independently.\n";
    } else {
      std::cerr << "Per-input output mode enabled: inputs will be combined for "
                   "training/scoring and outputs split per input.\n";
    }
    for (std::vector<PerInputOutputPlanner::Entry>::const_iterator it =
             entries.begin();
         it != entries.end(); ++it) {
      std::cerr << "  " << it->originalPath << " -> prefix " << it->prefix
                << std::endl;
    }
  }

  sourceTagDelimiter_ = PerInputOutputPlanner::sourceDelimiter();

  if (trainEach_) {
    for (std::vector<PerInputOutputPlanner::Entry>::const_iterator it =
             entries.begin();
         it != entries.end(); ++it) {
      PerInputOutputPlanner::FixedOutputs fixed =
          PerInputOutputPlanner::buildFixedOutputs(
              outputEachFolderPath_, *it, outputEachTargetPeptides_,
              outputEachDecoyPeptides_, outputEachTargetPsms_,
              outputEachDecoyPsms_);

      peptideResultFN_ = fixed.targetPeptides;
      decoyPeptideResultFN_ = fixed.decoyPeptides;
      psmResultFN_ = fixed.targetPsms;
      decoyPsmResultFN_ = fixed.decoyPsms;

      tabOutputFN_ =
          outputEachExtraTab_
              ? PerInputOutputPlanner::buildTemplateOutput(
                    outputEachFolderPath_, *it, tabOutputTemplateFN_)
              : "";
      xmlOutputFN_ =
          outputEachExtraXml_
              ? PerInputOutputPlanner::buildTemplateOutput(
                    outputEachFolderPath_, *it, xmlOutputTemplateFN_)
              : "";
      pepXMLOutputFN_ =
          outputEachExtraPepXml_
              ? PerInputOutputPlanner::buildTemplateOutput(
                    outputEachFolderPath_, *it, pepXMLOutputTemplateFN_)
              : "";
      weightOutputFN_ =
          outputEachExtraWeight_
              ? PerInputOutputPlanner::buildTemplateOutput(
                    outputEachFolderPath_, *it, weightOutputTemplateFN_)
              : "";
      proteinResultFN_ =
          outputEachExtraProtein_
              ? PerInputOutputPlanner::buildTemplateOutput(
                    outputEachFolderPath_, *it, proteinResultTemplateFN_)
              : "";
      decoyProteinResultFN_ =
          outputEachExtraDecoyProtein_
              ? PerInputOutputPlanner::buildTemplateOutput(
                    outputEachFolderPath_, *it, decoyProteinResultTemplateFN_)
              : "";
      reportPepXML_ = outputEachExtraPepXml_;

      perInputOutputSpecs_.clear();
      inputFNs_.assign(1, it->materializedPath);
      inputFN_ = it->materializedPath;
      useMixMax_ = useMixMaxBase_;

      if (VERB > 0) {
        std::cerr << "Processing input file: " << it->originalPath << std::endl;
      }
      if (!runCore()) {
        return 0;
      }
  ...
  for (std::vector<PerInputOutputPlanner::Entry>::const_iterator it =
    ...
    PerInputOutputPlanner::FixedOutputs fixed =
        PerInputOutputPlanner::buildFixedOutputs(
            ...
            outputEachDecoyPeptides_, outputEachTargetPsms_,
            outputEachDecoyPsms_);
    PerInputOutputSpec spec;
    spec.sourceKey = it->sourceKey;
    spec.prefix = it->prefix;
    spec.targetPsms = fixed.targetPsms;
    spec.decoyPsms = fixed.decoyPsms;
    spec.targetPeptides = fixed.targetPeptides;
    spec.decoyPeptides = fixed.decoyPeptides;
    perInputOutputSpecs_.push_back(spec);
  }

  std::vector<std::string> sourceKeys;
  sourceKeys.reserve(entries.size());
  for (std::vector<PerInputOutputPlanner::Entry>::const_iterator it =
           entries.begin();
       it != entries.end(); ++it) {
    sourceKeys.push_back(it->sourceKey);
  }

  ValidateTabFile validateTab;
  inputFN_ = validateTab.concatenateMultiplePINs(materializedInputPaths, sourceKeys);
  inputFNs_ = materializedInputPaths;
  useMixMax_ = useMixMaxBase_;
