...
//    postToPostHog("percolator_startup");
  ...
  if (tabInput_ && !readStdIn_) {
    try {
      inputFNs_ = InputFileResolver::expandPatterns(inputFNs_);
      inputFNs_ = InputFileResolver::deduplicateKeepFirst(inputFNs_);
      inputFNs_ = InputFileResolver::materializeGzipInputs(inputFNs_);
    } catch (const std::exception& e) {
      std::cerr << e.what() << std::endl;
      return 0;
    }
